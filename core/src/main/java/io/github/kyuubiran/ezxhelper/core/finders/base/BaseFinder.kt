...
abstract class BaseFinder<T, Self>(protected var sequence: Sequence<T>) : INamed {
    ...
    abstract fun newFinder(sequence: Sequence<T>): Self

    @Suppress("UNCHECKED_CAST")
    protected inline fun makeNewFinder(block: BaseFinder<T, Self>.() -> Sequence<T>) =
        (newFinder(block()) as BaseFinder<T, Self>) as Self
     ...
     */
    fun onEach(action: (T) -> Unit): Self = makeNewFinder { sequence.onEach(action) }
